...
A client texture is not necessarily the video resolution, and the three buffers of one client source need not match each other. The camera's `client_color_render_scale` and `client_aux_render_scale` set what it renders at ([wire-protocol.md](./wire-protocol.md)). The composite is always built at video dimensions, so a client rendering above them is supersampling: `DrawLayerNode` draws its fullscreen quad against the working framebuffer and the client texture's `GL_LINEAR` filter resolves the extra samples, which is what smooths a rendered silhouette that has no video edge to hide behind. An exact 2x is the scale that downsamples cleanly, since each destination pixel then lands on a 2x2 texel box.
