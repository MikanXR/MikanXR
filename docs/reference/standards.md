...
- Never compile a GObject/GstMeta-registering `.cpp` into both a plugin DLL and an executable that loads it; GLib's type registry is process-global and the duplicate registration fails at runtime (documented in `src/Programs/Tests/UnitTests/CMakeLists.txt`, which is where the case arises).
...
- Pixel sizes written into UI code are authored against a 96 DPI display and must be multiplied by `MkGui::getUiScale()` where they are used: button and image sizes, fixed panel widths, cursor offsets, and any width handed to `SetNextItemWidth`. Font sizes are the exception and take no multiply, since ImGui applies the same scale to every font size, including the ones `MkGuiScopedFont` pushes. The scale combines the display's content scale with the user's preference and changes while the app runs, so it is read at the point of use rather than cached. A window's own creation size is authored the same way and is scaled by `EditorWindow::startupWindow` through `IMkWindowContextManager::getPrimaryDisplayContentScale`, since there is no ImGui context yet at that point. Inside a node canvas the same rule holds, with one addition: positions and sizes that persist in a graph file stay in logical units and convert at `MkCanvas::toCanvasSpace`/`fromCanvasSpace` ([scripting.md](./scripting.md)). A dock layout is the one persisted size that is not stored logically: ImGui owns the `imgui_<name>.ini` format, so the layout keeps the scale it was arranged at and carries that scale in a `MikanLayout` ini entry, which `MkGuiContext::refreshDockLayoutScale` uses to re-proportion the splits when the two differ. This is the same reference-scale trick ImGui itself applies to table column widths.
